...
const OUTPUT_DIR_V04 = path.join(OUTPUT_DIR, 'daily_mtm', 'hedge_monitoring_calibration_v04');
...
function assertNoOverwrite() {
  ...
  if (existing.length) throw new Error(`Refusing to overwrite v0.4 outputs: ${existing.map(file => path.relative(REPO_ROOT, file)).join(', ')}`);
}
...
if (require.main === module) {
  try {
    ...
    console.error('Error calibrating passive hedge monitoring v0.4:', error.stack || error.message);
    process.exitCode = 1;
  }
}
