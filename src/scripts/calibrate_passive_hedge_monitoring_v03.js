...
const CONFIGS = [
  {
    id: 'v03a_drawdown_crisis_plus_var_stress',
    label: 'v0.3a: crisis = drawdown crisis + VaR stress',
    crisisRule: 'drawdown_crisis_plus_var_stress'
  },
  {
    id: 'v03b_drawdown_stress_plus_var_stress_plus_tail',
    label: 'v0.3b: crisis = drawdown stress + VaR stress + recent tail',
    crisisRule: 'drawdown_stress_plus_var_stress_plus_tail'
  },
  {
    id: 'v03c_var_or_ewma_crisis_with_drawdown_watch',
    label: 'v0.3c: crisis = VaR crisis or EWMA crisis with drawdown watch',
    crisisRule: 'acute_crisis_plus_drawdown_watch'
  }
];
...
const OUTPUT_DIR_V03 = path.join(OUTPUT_DIR, 'daily_mtm', 'hedge_monitoring_calibration_v03');
const OUTPUT_CONFIGS_JSON = path.join(OUTPUT_DIR_V03, 'threshold_sets_v03.json');
const OUTPUT_SUMMARY_CSV = path.join(OUTPUT_DIR_V03, 'calibration_v03_summary.csv');
const OUTPUT_SUMMARY_JSON = path.join(OUTPUT_DIR_V03, 'calibration_v03_summary.json');
const OUTPUT_YEARLY_CSV = path.join(OUTPUT_DIR_V03, 'yearly_v03_comparison.csv');
const OUTPUT_TRANSITIONS_CSV = path.join(OUTPUT_DIR_V03, 'transition_v03_comparison.csv');
const OUTPUT_SIGNALS_CSV = path.join(OUTPUT_DIR_V03, 'signals_v03_recommended.csv');
...
function buildSignals(rows, config) {
  ...
  return rows.map(row => {
    ...
    const durationState = severity(underwaterDuration, THRESHOLDS.underwaterDurationDays, 'gte');
    const cappedDurationState = capState(durationState, 'stress');
    const damageState = maxState([drawdownState, cappedDurationState]);
    ...
    const tailState = severity(dailyReturnPct, THRESHOLDS.dailyReturnPct, 'lte');
    const tailClusterState = recentTailLossDays >= TAIL_CLUSTER_COUNT ? 'stress' : 'normal';
    const acuteState = maxState([varState, ewmaState, tailState, tailClusterState]);

    let alertState = acuteState;
    if (RANK[damageState] >= RANK.watch && alertState === 'normal') alertState = 'watch';
    if (RANK[damageState] >= RANK.stress && alertState === 'watch') alertState = 'stress';
    if (RANK[damageState] >= RANK.crisis && RANK[acuteState] >= RANK.stress) alertState = 'stress';

    const crisisReason = crisisReasonFor(config, {
      drawdownState,
      varState,
      ewmaState,
      recentTailLossDays
    });
    if (crisisReason) alertState = 'crisis';
    ...
    return {
      ...
      acute_state: acuteState,
      alert_state: alertState,
      drawdown_state: drawdownState,
      duration_state: cappedDurationState,
      var_state: varState,
      ewma_state: ewmaState,
      tail_state: maxState([tailState, tailClusterState]),
...
function crisisReasonFor(config, s) {
  if (config.crisisRule === 'drawdown_crisis_plus_var_stress') {
    return s.drawdownState === 'crisis' && RANK[s.varState] >= RANK.stress
      ? 'drawdown_crisis_plus_var_stress'
      : null;
  }
  if (config.crisisRule === 'drawdown_stress_plus_var_stress_plus_tail') {
    return RANK[s.drawdownState] >= RANK.stress
      && RANK[s.varState] >= RANK.stress
      && s.recentTailLossDays >= TAIL_CLUSTER_COUNT
      ? 'drawdown_stress_plus_var_stress_plus_tail'
      : null;
  }
  if (config.crisisRule === 'acute_crisis_plus_drawdown_watch') {
    return RANK[s.drawdownState] >= RANK.watch
      && (s.varState === 'crisis' || s.ewmaState === 'crisis')
      ? 'acute_crisis_plus_drawdown_watch'
      : null;
  }
  return null;
}
...
function summarizeConfig(config, signals, trans) {
  ...
  const stressYearLargeDrawdowns = stressYears.filter(s => s.rolling_drawdown_pct !== null && s.rolling_drawdown_pct <= -35);
  const stressYearFalseNegatives = stressYearLargeDrawdowns.filter(s => RANK[s.alert_state] < RANK.watch);
  ...
  return {
    ...
    stressYearsFalseNegativePct: roundNumber(stressYearFalseNegatives.length / Math.max(stressYearLargeDrawdowns.length, 1) * 100),
    recommended: config.id === 'v03b_drawdown_stress_plus_var_stress_plus_tail'
  };
}
...
function yearlySummary(config, signals, trans) {
  return YEARS.map(y => {
    const rows = signals.filter(s => s.year === y.year);
    const yearTrans = trans.filter(t => t.year === y.year);
...
function buildMarkdown(summaries, yearly, recommendation) {
  return [
    ...
    '',
    markdownTable(summaries, ['configId', 'normalDays', 'watchDays', 'stressDays', 'crisisDays', 'stressOrCrisisPct', 'crisisPct', 'damageStressOrCrisisPct', 'transitionCount', 'maxStressEpisodeDays', 'maxCrisisEpisodeDays', 'largeDrawdownLeadPct', 'largeDrawdownFalseNegativePct', 'bullRecoveryStressOrCrisisPct', 'bullRecoveryCrisisPct', 'recommended']),
    '',
    '## Recommendation',
    '',
    `Recommended v0.3 set: ${recommendation.configId}.`,
    '',
...
function assertNoOverwrite() {
  const outputs = [OUTPUT_CONFIGS_JSON, OUTPUT_SUMMARY_CSV, OUTPUT_SUMMARY_JSON, OUTPUT_YEARLY_CSV, OUTPUT_TRANSITIONS_CSV, OUTPUT_SIGNALS_CSV, OUTPUT_FINDINGS_MD];
  const existing = outputs.filter(file => fs.existsSync(file));
  if (existing.length) throw new Error(`Refusing to overwrite v0.3 outputs: ${existing.map(file => path.relative(REPO_ROOT, file)).join(', ')}`);
}
...
function main() {
  ...
  fs.writeFileSync(OUTPUT_CONFIGS_JSON, `${JSON.stringify({ thresholds: THRESHOLDS, configs: CONFIGS }, null, 2)}\n`, 'utf8');
...
if (require.main === module) {
  try {
    ...
    console.error('Error calibrating passive hedge monitoring v0.3:', error.stack || error.message);
    process.exitCode = 1;
  }
}
